...
public class CameraHelper implements Camera.PreviewCallback {
    ...
    private boolean isCameraInitialized = false;//相机是否初始化过
    ...
    private void printStackTrace() {
        ...
        if (stackElements != null) {
            for (int i = 0; i < stackElements.length; i++) {
                System.out.println("stack" + stackElements[i]);
            }
        }
    }
    ...
    private Camera.Size getBestSupportedSize(List<Camera.Size> sizes) {
        ...
        if (null != previewViewSize) {
            int surfaceWidth = previewViewSize.x;
            int surfaceHeight = previewViewSize.y;
            ...
            return getBestSupportedSize(sizes, previewViewSize);
    }
